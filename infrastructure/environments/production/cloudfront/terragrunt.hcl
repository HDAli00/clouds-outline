...
dependency "s3" {
  config_path  = "../s3"
  mock_outputs = { static_bucket_id = "outline-production-static-assets" }
}
dependency "acm" {
  config_path  = "../acm"
  mock_outputs = { certificate_arn = "arn:aws:acm:us-east-1:123:certificate/xxx" }
}
terraform { source = "../../../_modules//cloudfront" }
inputs = {
  ...
  price_class      = "PriceClass_All"  # production: full global CDN
}
